...
async function handleEdit(record: any) {
  ...
  form.contactInfo = record.contactInfo
  form.deposit = record.deposit
}

async function handleDelete(record: any) {
  try {
    await request(`/api/member/remove?id=${record.id}`, {
      method: 'POST',
    })
...
async function confirmForm(e: Event) {
  ...
  try {
    const url = isEdit ? '/api/member/update' : '/api/member/add'
    await request(url, {
      method: 'POST',
      body: form,
    })
      ...
      <NModal v-model:show="showModal" :show-icon="false" preset="dialog" :title="title">
        ...
        >
          ...
          <NFormItem label="联系方式">
            <NInput v-model:value="form.contactInfo" />
          </NFormItem>
          <NFormItem label="余额">
            <NInput v-model:value="form.deposit" />
          </NFormItem>
        </NForm>
